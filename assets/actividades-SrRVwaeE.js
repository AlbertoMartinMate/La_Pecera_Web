import"./main-CFXtgMme.js";import{t as e}from"./galeria-B6UmQMRF.js";var t=document.getElementById(`acti-hero-stories`);t&&e.meses.flatMap(e=>e.fotos).slice(-5).forEach(e=>{let n=document.createElement(`div`);n.className=`acti-hero__story`,n.innerHTML=`<img src="${e}" alt="" loading="lazy" />`,t.appendChild(n)});var n=document.getElementById(`btn-zoom`),r=document.getElementById(`modal-zoom`);if(n&&r){let e=r.querySelector(`.modal__close`),t=r.querySelector(`.modal__backdrop`);function i(){r.classList.add(`is-open`),document.body.style.overflow=`hidden`}function a(){r.classList.remove(`is-open`),document.body.style.overflow=``}n.addEventListener(`click`,i),e.addEventListener(`click`,a),t.addEventListener(`click`,a),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&a()})}function i(e){let t=e.fotos.slice(0,6),n=document.createElement(`div`);return n.className=`ganadores__month-card`,n.setAttribute(`role`,`button`),n.setAttribute(`tabindex`,`0`),n.setAttribute(`aria-label`,`Ver fotos de ${e.nombre}`),n.innerHTML=`
